...
def find_last_checkpoint(save_dir, net_type='G'):
    ...
    if file_list:
        ...
        for file_ in file_list:
            epoch_current = re.findall(r"(\d+)_{}.pth".format(net_type), file_)
            epoch_exist.append(int(epoch_current[0]))
        init_epoch = max(epoch_exist)
        init_path = os.path.join(save_dir, '{}_{}.pth'.format(init_epoch, net_type))
    else:
        init_epoch = 0
        init_path = None
...
def save(opt):
    ...
    if os.path.exists(opt_path_copy) is False:
        os.makedirs(opt_path_copy)
